...
impl Wires
{
    fn on_segment(&self, p: (i64, i64), q: (i64, i64), r: (i64, i64)) -> bool
    { 
    ...
    pub fn do_intersect(&self, p1: (i64, i64), q1: (i64, i64), p2: (i64, i64), q2: (i64, i64)) -> bool
    { 
    ...
    fn get_nearest_intersection_distance(&self, filename: &str) -> i64
    {
        ...
        let mut pathlenght: (i64, i64) = (0, 0);
        ...
        for w1step in &w1 {
            ...
            for w2step in &w2 {
                //println!("pathlenght: {} {}", pathlenght.0, pathlenght.1);
                let w2LineEnd = ((currentpoints.1).0 + w2step.0, (currentpoints.1).1 + w2step.1);
                if self.do_intersect(currentpoints.0, w1LineEnd, currentpoints.1, w2LineEnd) {
                    let r = self.count_intersection(currentpoints.0, w1LineEnd, currentpoints.1, w2LineEnd);
                    let p1 = pathlenght.0 + (r.0 - (currentpoints.0).0).abs() + (r.1 - (currentpoints.0).1).abs();
                    let p2 = pathlenght.1 + (r.0 - (currentpoints.1).0).abs() + (r.1 - (currentpoints.1).1).abs();
                    if closest > (p1 + p2) && (r.0.abs() + r.1.abs()) != 0 && (pathlenght.0 + pathlenght.1) != 0 {
                        closest = p1 + p2;
                    }
                }
                pathlenght.1 += w2step.0.abs() + w2step.1.abs();
                currentpoints.1 = w2LineEnd;
            }
            currentpoints.1 = (0, 0);
            pathlenght.1 = 0;
            pathlenght.0 += w1step.0.abs() + w1step.1.abs();
